# Taken from: https://github.com/koulanurag/ma-gym/blob/master/ma_gym/envs/utils/draw.py with minor modifications

from PIL import Image, ImageDraw
def get_cell_sizes(cell_size):
...
def draw_grid_fixed(rows, cols, image_size=(65,65), fill='white', line_color='black'):
    h, w = image_size
    cell_h = h//rows
    cell_w = w//cols
    return draw_grid(rows,cols,(cell_h,cell_w),fill,line_color), [cell_h,cell_w]

def draw_grid(rows, cols, cell_size=50, fill='white', line_color='black'):
...
def fill_cells(image, pos, cell_size=None, fill='black', margin=0):
    assert cell_size is not None and 0 <= margin <= 1
    cell_size_x, cell_size_y = get_cell_sizes(cell_size)
    for p in pos:
        col, row = p
...
def draw_cell_outline(image, pos, cell_size=50, fill='black', width=1):
    cell_size_x, cell_size_y = get_cell_sizes(cell_size)
    col, row = pos
    row, col = row * cell_size_x, col * cell_size_y
    ImageDraw.Draw(image).rectangle([(row, col), (row + cell_size_x, col + cell_size_y)], outline=fill, width=width)
